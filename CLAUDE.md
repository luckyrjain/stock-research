...
own `sector` field (`tools/nse_tools.py::get_stock_quote` → `info.get("sector")`, a fixed
GICS-like taxonomy):
...
- **Cyclical** (`Basic Materials`, `Energy`, `Industrials`, `Consumer Cyclical`): technical and
  volume weighted up — price/volume momentum is more informative for a cyclical business than
  for a steady compounder.
