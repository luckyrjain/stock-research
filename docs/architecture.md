...
- `tools/securities_master.py::get_full_securities_master()` (used by `resolve_symbol` above)
  ...
  deduped by ISIN.
- **Broker API sync** (`portfolio/kite_sync.py` / `hdfc_sync.py` / `paytm_sync.py`, six
  ...
  Zerodha and Paytm Money implement the identical `get_login_url(api_key)` /
  ...
  browser-redirect OAuth-style logins), so `_broker_sync_module(broker)` dispatches on the path
  parameter with a plain `if/elif`. **HDFC Securities' real login has no browser redirect at
  all** — a 5-step API flow this app itself drives directly (`GET /login` → `POST
  /login/validate` with the user's own HDFC username/password → `POST /twofa/validate` with an
  OTP → `GET /authorise?consent=true` → `POST /access-token`), served by two dedicated endpoints
  (`login-start`/`verify-otp`) instead of the shared `login-url`/`connect` pair, which reject it
  outright. `sync_account(engine, account_id, access_token, api_key)` is still the one function
  every module (including HDFC's) exposes identically — only the login half diverges. The actual
  "write a normalized holding/trade into `assets`/`holdings`/`valuations`/`transactions`" logic
  lives once, in `portfolio/broker_sync_common.py`, which every module calls into after
  ...
  `core/crypto.py`). `POST .../login-url` (Zerodha/Paytm Money) doubles as "register credentials"
  (first call) and "resume the OAuth handshake" (later calls, credential fields omitted) in one
  endpoint; re-registering different credentials clears any stale access token rather than leaving
  `connections` reporting a connection that can no longer actually authenticate. HDFC's
  `login-start` follows the same both-or-neither credential semantics.
...
`db.models.metadata` directly — no second, Alembic-specific model layer). Nine revisions today:
...
constraint — DB-enforced broker-trade dedup, see "Broker API sync" below),
`35f10ea4dac3_add_broker_connections_background_sync_.py` (`broker_connections.sync_status`/
`.last_sync_summary`/`.last_sync_error` — backs the background-sync poll, same section), and
`976659233671_add_broker_connections_pending_token_id_.py` (`broker_connections.pending_token_id`
— HDFC Securities' real, non-redirect login needs it across its OTP/consent/access-token steps,
same section) — each
