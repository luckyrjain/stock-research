...
`db.models.metadata` directly — no second, Alembic-specific model layer). Six revisions today:
...
Zerodha-only shape at the time), and `6c43f4a2d489_add_broker_connections_per_account_api_.py`
(`api_key`/`api_secret_enc` — the per-connection-credential redesign) — each verified to
`upgrade head`/`downgrade base` cleanly against an isolated scratch database before landing.
**Existing deployments must `alembic stamp 0001`, then `upgrade head`,** — they already have
those 11 tables (via `db/schema.sql` or a pipeline's own `--setup-db`), so replaying `CREATE
TABLE` would fail on the first statement; the five later revisions apply normally via `upgrade
