...
`db.models.metadata` directly — no second, Alembic-specific model layer). Seven revisions today:
...
(`api_key`/`api_secret_enc` — the per-connection-credential redesign), and
`b7cf5b79ce66_add_transactions_external_ref_for_.py` (`transactions.external_ref` + its unique
constraint — DB-enforced broker-trade dedup, see "Broker API sync" below) — each verified to
...
TABLE` would fail on the first statement; the six later revisions apply normally via `upgrade
