  ...
  if (rank === 1) {
    return (
      <div className="w-6 h-6 rounded-full bg-accent flex items-center justify-center
                      text-[10px] font-black text-bg shadow-sm shadow-accent/30">
        1
      </div>
    );
  }
...
export default function MarketPicksDashboard({ picks, generatedAt, fromCache, onRescan, pricesLastUpdated }: Props) {
  ...
  return (
    <div className="animate-fade-up">
      ...
      <p className="text-[11px] text-muted leading-relaxed mt-4 px-1">
        These picks, ratings, and trade levels are generated by an AI model from public news and
        market data and are for informational purposes only — not investment advice, and
        AlphaPulse is not registered with SEBI as a Research Analyst or Investment Adviser. Do your
        own research and consult a SEBI-registered adviser before trading.
      </p>
    </div>
  );
}
