...
from routes/_shared.py rather than reaching into `api` for them — see
routes/watchlist.py's own docstring for why (a real dependency, not the
ordering-coincidence `import api` used to be).

The broker login/connect/sync endpoints live in routes/broker_sync.py, not
here — the CRUD half (this file) and the broker-sync half had grown large
enough to warrant their own files, the next increment of the same split
this codebase already applied to watchlist/positions (see
backend/CLAUDE.md's "Route module extraction" section). broker_sync.py
imports `_owned_account_id`/`_owned_profile_id` from here rather than
duplicating them, same "second domain imports the first domain's
primitive" shape as routes/positions.py importing from routes/watchlist.py.
"""
import json
